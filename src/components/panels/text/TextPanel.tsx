...
export const TextPanel: React.FC = () => {
  ...
  const anims = ["Tanpa anim", "Pan", "Denyut", "Float", "Shake"];
  return (
      ...
      <section className="bg-panel p-4 rounded-xl border border-gray-800">
        ...
        >
          <div className="flex gap-2">
            <Button
              onClick={() => activeTextId && moveTextDown(activeTextId)}
              className="flex-1"
            >
              ↓ Mundur
            </Button>
            <Button
              onClick={() => activeTextId && moveTextUp(activeTextId)}
              className="flex-1"
            >
              ↑ Maju
            </Button>
          ...
          >
            {fonts.map((f) => (
              <option key={f} value={f} className="bg-gray-900 text-gray-200">
                {f}
              </option>
            ))}
          </select>
          <div className="flex gap-2">
            ...
            >
              <option value="300" className="bg-gray-900 text-gray-200">
                Light
              </option>
              <option value="400" className="bg-gray-900 text-gray-200">
                Normal
              </option>
              <option value="500" className="bg-gray-900 text-gray-200">
                Medium
              </option>
              <option value="700" className="bg-gray-900 text-gray-200">
                Bold
              </option>
              <option value="900" className="bg-gray-900 text-gray-200">
                Black
              </option>
            </select>
            <select
              ...
              className="flex-1 bg-gray-900 border border-gray-800 p-2 rounded text-gray-200 focus:outline-none focus:border-accent cursor-pointer text-xs"
            >
              <option value="normal" className="bg-gray-900 text-gray-200">
                Regular
              </option>
              <option value="italic" className="bg-gray-900 text-gray-200">
                Italic
              </option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              min="10"
              max="200"
              value={activeText?.size || 80}
              onChange={(e) =>
                updateActiveText({ size: Number(e.target.value) })
              }
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) =>
                updateActiveText({ opacity: Number(e.target.value) })
              }
              className="flex-1 accent-accent cursor-pointer"
            />
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {effects.map((fx) => (
                <Button
                  key={fx}
                  active={activeText?.effect === fx}
                  onClick={() => updateActiveText({ effect: fx })}
                >
                  {fx}
                </Button>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {anims.map((a) => (
                <Button
                  key={a}
                  active={activeText?.animation === a}
                  onClick={() => updateActiveText({ animation: a })}
                >
                  {a}
                </Button>
              ))}
